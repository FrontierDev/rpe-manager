...
RPEngine addon binaries and version discovery are handled directly through **GitHub Releases in `FrontierDev/rpe2`**.

Esarus is not part of the RPEngine release-discovery or binary-download path.

Datasets remain a separate distribution system and continue to be hosted and served through the Esarus catalogue/backend.

The intended RPEngine distribution flow is:
...
clean RPEngine2 release ZIP asset
    ↓
RPEngine Manager queries GitHub Releases directly
    ↓
verified staged installation/update
~~~

The intended dataset distribution flow remains:

~~~
esarus.net catalogue/backend
    ↓
dataset metadata + compatibility requirements + payload
    ↓
RPEngine Manager
    ↓
RPE external-management protocol
~~~
...
- release artifact hashing suitable for verification by the Manager;
...
GitHub Releases are the canonical source of:

- available RPEngine versions;
- release channel/prerelease state;
- release notes;
- release assets;
- RPEngine binary downloads.

The Manager must not install directly from a source branch checkout or GitHub repository archive.

## Manager release discovery

The Manager queries the `FrontierDev/rpe2` GitHub Releases API directly.

It must determine available versions and update availability from GitHub release/tag metadata rather than from Esarus.
...
- release asset integrity metadata where available;
- release notes where useful to the UI.

If additional release integrity data is required, it should be published as part of the GitHub Release itself, for example a checksum file/signature asset generated by the release workflow.

## Update channels

The release model must support explicit channels, for example:
...
Channel behaviour should be derived deterministically from GitHub release/tag metadata, for example through semantic-version prerelease identifiers and/or GitHub's prerelease state.
...
No Esarus configuration change is required when a new RPEngine version is released.

## Esarus dataset compatibility role

Esarus continues to own dataset catalogue metadata and payload hosting.

Dataset metadata may declare compatibility requirements such as:
...
When a dataset requires a newer RPEngine version, the Manager:

1. reads the requirement from Esarus dataset metadata;
2. compares it against the locally installed RPEngine version;
3. queries GitHub Releases directly for a compatible RPEngine release;
4. offers/performs the required RPEngine update through the normal Phase 5 installation path.

Esarus therefore defines **what a dataset requires**, but not **what RPEngine releases exist**.

## Manager implementation

Implement:

- direct GitHub Releases discovery for `FrontierDev/rpe2`;
- comparison of the installed TOC version against GitHub release versions available on the selected channel;
- download of the exact GitHub Release asset;
- release hash/checksum verification before extraction or installation;
...
- compatibility-driven addon updates when an Esarus-hosted dataset requires a newer RPEngine version.
...
- the downloaded release asset matches its expected checksum/hash;
- the archive has the expected RPEngine2 package structure;
- `RPEngine2.toc` exists;
- the packaged addon version matches the selected GitHub Release/tag;
- extraction completed successfully in a staging location.

A failed download, checksum/hash check, archive validation or extraction must leave the existing addon installation unchanged.
...
The ZIP is attached to a GitHub Release and GitHub Releases are the canonical source for RPEngine versions and binaries.

Manager discovers available RPEngine versions directly from FrontierDev/rpe2 GitHub Releases.

Publishing a new RPEngine GitHub Release requires no Esarus metadata update.

Manager can install RPEngine into a valid WoW installation from the selected GitHub Release asset.

Manager verifies the downloaded release checksum/hash before modifying the addon installation.

Manager detects the resulting installed version from RPEngine2.toc.

Manager can update an existing installation to a later release on the selected update channel.

A failed download, integrity verification, extraction or staged validation does not destroy the previous installation.

An Esarus-hosted dataset compatibility requirement can request a newer RPEngine version, which the Manager resolves against GitHub Releases.

WoW running prevents installation changes.

Datasets continue to be hosted/downloaded through Esarus rather than GitHub Releases.
~~~
